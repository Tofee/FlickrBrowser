...
Rectangle {
    ...
    function tileUrl(layerID, tx, ty) {
        if (ty < 0 || ty > maxTileNo) {
            return Qt.resolvedUrl("noimage.png");
        } else {
            /*
            if (tileserverPort != 0) {
            ...
            */
            return F.getMapTile(url, tx, ty, zoomLevel);
        }
    }
            ...
            Rectangle {
                ...
                */
                ...
                Image {
                    ...
                    id: img
                    anchors.fill: parent;
                    source : tileUrl(pinchmap.mapLayer, tileX, tileY)
                    asynchronous : true
                }
...
*/
    ...
    MouseArea {
        ...
        onWheel: {
            var scale = Math.round(wheel.angleDelta.y / 120.) + pinchmap.zoomLevel;
            pinchmap.setZoomLevelPoint(scale, wheel.x, wheel.y);
        }
    }
}
